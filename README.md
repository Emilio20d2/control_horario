...
El modo de autenticación local utiliza credenciales configurables. Copia el archivo `.env.example` a `.env.local` y define, como mínimo, las siguientes variables:
...
1. Implementa un nuevo adaptador que cumpla la interfaz `DatabaseAdapterDefinition` (consulta `src/lib/database/index.ts`).
2. Sustituye la lógica de `useAuth.tsx` por tu proveedor de identidad preferido.
